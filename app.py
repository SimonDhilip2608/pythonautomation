...
with st.form("log_search_form"):
    ...
    with col1:
        ...
        start_time = st.datetime_input("Last Transaction Start Date/Time")
    
    with col2:
        task_name = st.text_input("Task Name")
        end_time = st.datetime_input("Last Transaction End Date/Time")
...
if submit_button:
    ...
    if not work_order or not task_name:
        ...
        if logs is None:
            ...
            status_text.text("Analyzing logs with Google Gemini...")
            error_analysis = gemini_service.analyze_logs(logs)
...
with st.sidebar:
    ...
    **Log Advisor** helps you:
    ...
    - Analyze logs for errors using Google Gemini
    ...
    gemini_status = ":green[Connected]" if gemini_service.is_configured() else ":red[Not Configured]"
